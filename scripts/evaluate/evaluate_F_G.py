#!/usr/bin/env python3
"""
DiatomScanNet F Genus Baseline Prediction
...
from diatom_cascade.prediction import load_taxonomy_tree, flat_bottom_up_lookup_from_genus as infer_upper_levels_from_genus
from diatom_cascade.models import FlatClassifier

# Override checkpoint path for this model
Config.CHECKPOINT_PATH = Config.get_checkpoint_path("F-G")


def load_model(checkpoint_path):
    """Load trained F Genus model"""
    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(f"Model file not found: {checkpoint_path}")
    
    checkpoint = load_checkpoint(checkpoint_path, Config.DEVICE)
    
    label_encoder = load_label_encoder(checkpoint, 'genus_names')
    ...
    print(f"✅ Model loaded ({config['NUM_CLASSES']} genera)")
...
class DiatomDataset(Dataset):
    """Dataset for genus-level classification"""
    ...
    def __getitem__(self, idx):
        ...
        # Encode label
        genus = str(row['genus'])
        label_id = self.label_encoder.transform([genus])[0]
        
        # Return image, label_id, original genus name, and upper levels if available
        result = [image, label_id, genus]
...
def prepare_test_data(label_encoder):
    """Prepare test data for genus-level evaluation"""
    ...
    df = df[df['genus'].notna() & (df['genus'] != '')].copy()
...
def evaluate_model(model, test_loader, label_encoder):
    """Evaluate F Genus model on test set, including upper level inference"""
    model.eval()
    all_preds = []
    all_labels = []
    all_true_labels = []
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Evaluating"):
            ...
            true_genus_labels = batch[2]  # Original genus names (list)
            ...
            all_true_labels.extend(true_genus_labels)
            all_pred_labels.extend([label_encoder.classes_[p] for p in preds.cpu().numpy()])
            all_probabilities.extend(probabilities.cpu().numpy())
            
            # Look up upper levels from predicted genus using taxonomy tree
            if has_taxonomy:
                for i in range(batch_size):
                    pred_genus = label_encoder.classes_[preds[i].item()]
                    pred_class, pred_order, pred_family = infer_upper_levels_from_genus(
                        pred_genus, taxonomy_tree, mappings
                    )
...
def generate_report(eval_results, label_encoder, save_path):
    ...
    # Extract results
    true_genus_labels = eval_results['all_true_labels']
    pred_genus_labels = eval_results['all_pred_labels']
    
    # Calculate genus-level metrics
    genus_accuracy = accuracy_score(eval_results['all_labels'], eval_results['all_preds'])
    genus_f1_macro = f1_score(eval_results['all_labels'], eval_results['all_preds'], average='macro', zero_division=0)
    genus_f1_weighted = f1_score(eval_results['all_labels'], eval_results['all_preds'], average='weighted', zero_division=0)
    
    # Always print main results (concise)
    num_test_samples = len(eval_results['all_labels'])
    print(f"Genus: Acc {genus_accuracy:.4f} ({genus_accuracy*100:.2f}%), F1-macro {genus_f1_macro:.4f}, F1-weighted {genus_f1_weighted:.4f} (N={num_test_samples})")
    ...
    report_data = {
        ...
        'test_samples': len(true_genus_labels),
        'num_classes': len(label_encoder.classes_)
    }
    
    report_filename = 'F_G_evaluation_report.json'
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate F-G flat model')
    
    # Print separator for model identification
    print("\n" + "=" * 80)
    print("Evaluating Model: F-G")
    print("=" * 80 + "\n")
    
    # Check model file
    if not Path(Config.CHECKPOINT_PATH).exists():
        print(f"Model file not found: {Config.CHECKPOINT_PATH}")
        print(f"Please train the F Genus model first")
        print(f"Expected path: outputs/checkpoints/best_F_G_model.pth")
        return
    ...
    def save_predictions(eval_results, label_encoder, test_df, output_path):
        """保存预测结果为JSON（F-G模型：保存genus名称，错误传播分析脚本会转换为ID）"""
        true_genus_names = eval_results['all_true_labels']
        pred_genus_names = eval_results['all_pred_labels']
        filenames = test_df['filename'].astype(str).tolist()
        if len(filenames) != len(true_genus_names):
            raise RuntimeError("Prediction count does not match the F-G test manifest")
        
        # 从eval_results获取真实的上层级
        ...
        predictions_dict = {
            ...
            'true_genus': true_genus_names,
            'pred_class': pred_classes,
            'pred_order': pred_orders,
            'pred_family': pred_families,
            'pred_genus': pred_genus_names,
    ...
    # Save predictions
    predictions_path = Config.EVAL_DIR / 'F_G_predictions.json'
