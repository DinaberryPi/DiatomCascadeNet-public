#!/usr/bin/env python3
"""
DiatomScanNet H-COFG Model Evaluation Script
Evaluate Class→Order→Family→Genus hierarchical model on test set
"""
...
Config.MODEL_PATH = Config.get_checkpoint_path("H-COFG")
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate H-COFG hierarchical model')
    
    # Print separator for model identification
    print("\n" + "=" * 80)
    print("Evaluating Model: H-COFG")
    print("=" * 80 + "\n")
    
    # Load data
    df, class_encoder, order_encoder, family_encoder, genus_encoder = load_and_prepare_data()
    
    _, _, test_df = load_split_manifests(Config.DATA_ROOT, "H-COFG")
    validate_images(test_df, Config.IMAGES_DIR,
                    Config.OUTPUT_DIR / "preflight" / "H-COFG_test_images.csv")
    
    # Data transforms (use unified config)
    test_transforms = get_preprocess_transforms()
    
    # Create test dataset
    test_dataset = HCOFGDataset(test_df, class_encoder, order_encoder, family_encoder, genus_encoder, test_transforms)
    ...
    model.eval()
    print(f"✅ Model loaded ({num_classes} classes, {num_orders} orders, {num_families} families, {num_genera} genera)")
    ...
    with torch.no_grad():
        for images, class_labels, order_labels, family_labels, genus_labels in tqdm(test_loader, desc="Evaluating"):
            images = images.to(Config.DEVICE)
            
            class_logits, order_logits, family_logits, genus_logits, _, _, _ = model(images)
            
            # Method 1: Greedy hierarchical prediction (with mask constraints)
            pred_class_hier, pred_order_hier, pred_family_hier, pred_genus_hier = hierarchical_predict(
            ...
            pred_class_argmax, pred_order_argmax, pred_family_argmax, pred_genus_argmax = argmax_independent_predict(
                class_logits, order_logits, family_logits=family_logits, genus_logits=genus_logits
            )
            
            # Method 3: Beam search prediction (globally constrained)
            pred_class_beam, pred_order_beam, pred_family_beam, pred_genus_beam = beam_search_hierarchical_predict(
                class_logits, order_logits,
                family_logits=family_logits, genus_logits=genus_logits,
                M_class_order=M_class_order, M_order_family=M_order_family, M_family_genus=M_family_genus,
    ...
    # Calculate metrics for beam search prediction (globally constrained)
    ...
    num_test_samples = len(all_class_true)
    print(f"Greedy: C {class_acc_hier:.4f} | O {order_acc_hier:.4f} | F {family_acc_hier:.4f} | G {genus_acc_hier:.4f} (N={num_test_samples})")
    print(f"Argmax: C {class_acc_argmax:.4f} | O {order_acc_argmax:.4f} | F {family_acc_argmax:.4f} | G {genus_acc_argmax:.4f} (N={num_test_samples})")
    print(f"Beam:   C {class_acc_beam:.4f} | O {order_acc_beam:.4f} | F {family_acc_beam:.4f} | G {genus_acc_beam:.4f} (N={num_test_samples})")
    
    # Save report with both methods
    report = {
    ...
    report_path = Config.EVAL_DIR / 'H_COFG_evaluation_report.json'
    ...
    def save_predictions(all_predictions, test_df, output_path):
        ...
        if len(filenames) != len(all_predictions['true_genus']):
            raise RuntimeError("Prediction count does not match the H-COFG test manifest")
        predictions_dict = {
            'filename': filenames,
    ...
    predictions_path = Config.EVAL_DIR / 'H_COFG_greedy_predictions.json'
