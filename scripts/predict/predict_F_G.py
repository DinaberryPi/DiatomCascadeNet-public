...
def load_model(checkpoint_path):
    ...
    print(f"Loading F Genus model: {checkpoint_path}")
    ...
    print(f"✅ F Genus model loaded successfully")
...
def predict_single_image(model, image_path, label_encoder, top_k=3):
    ...
    return {
        ...
        'predictions': [{'genus': name, 'confidence': float(prob)} for name, prob in zip(top_classes, top_probs)]
    }
...
def predict_batch(model, image_dir, label_encoder, output_csv=None):
    ...
    for image_path in image_files:
        try:
            ...
            predicted_genus = label_encoder.classes_[top_idx.item()]
            ...
            print(f"  {image_path.name}: {predicted_genus} ({confidence:.4f})")
...
def main():
    parser = argparse.ArgumentParser(description='DiatomScanNet F Genus Baseline Prediction')
    
    # Print separator for model identification
    print("\n" + "=" * 80)
    print("Predicting with Model: F-G")
    print("=" * 80 + "\n")
