...
- Single-head classification (Class level only)
- Baseline model for hierarchical comparison
"""
...
def get_class_weights_and_sampler(df, target_col='class'):
...
def prepare_data():
    ...
    # Create sampler
    _, train_sampler, _ = get_class_weights_and_sampler(train_df, 'class')
...
def main():
    parser = argparse.ArgumentParser(description='Train F-C flat model')
    ...
    for epoch in pbar:
        ...
        if is_best:
            ...
            }, Config.CHECKPOINT_DIR / 'best_F_C_model.pth')
    ...
    history_path = Config.LOG_DIR / 'F_C__training_history.json'
