...
class Config:
    ...
    # Pretrained model (use H-CO hierarchical model)
    PRETRAINED_MODEL_PATH = CHECKPOINT_DIR / "best_H_CO_model.pth"
...
class ThreeLevelDataset(Dataset):
    """"""
    def __init__(self, df, class_encoder, order_encoder, family_encoder, transform=None):
...
def load_and_prepare_data():
    ...
    return df, class_encoder, order_encoder, family_encoder

def create_data_loaders(df, class_encoder, order_encoder, family_encoder):
    """"""
    # 
    ...
    # 
    train_dataset = ThreeLevelDataset(train_df, class_encoder, order_encoder, family_encoder, train_transforms)
    val_dataset = ThreeLevelDataset(val_df, class_encoder, order_encoder, family_encoder, val_transforms)
    test_dataset = ThreeLevelDataset(test_df, class_encoder, order_encoder, family_encoder, val_transforms)
...
def train_epoch(model, train_loader, criterion, optimizer, device):
    ...
    for images, class_labels, order_labels, family_labels in train_loader:
        ...
        optimizer.zero_grad()
        class_logits, order_logits, family_logits, class_probs, order_probs = model(images)
        ...
        weights = TrainingConfig.LOSS_WEIGHTS["H-COF"]
        total_loss_batch = weights["class"] * class_loss + weights["order"] * order_loss + weights["family"] * family_loss
        ...
        with torch.no_grad():
            pred_class, pred_order, pred_family = hierarchical_predict(
                ...
                family_logits=family_logits.detach(),
    ...
    return avg_loss, class_acc, order_acc, family_acc, avg_class_loss, avg_order_loss, avg_family_loss
...
def validate_epoch(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for images, class_labels, order_labels, family_labels in val_loader:
            images = images.to(device)
            class_labels = class_labels.to(device)
            order_labels = order_labels.to(device)
            family_labels = family_labels.to(device)
    ...
    return avg_loss, class_acc, order_acc, family_acc, class_f1, order_f1, family_f1, avg_class_loss, avg_order_loss, avg_family_loss

# Plotting function moved to analyze/plot_individual_training_curves.py

def main():
    parser = argparse.ArgumentParser(description='Train H-COF hierarchical model')
    ...
    df, class_encoder, order_encoder, family_encoder = load_and_prepare_data()
    ...
    history = {
        ...
        'val_class_f1': [], 'val_order_f1': [], 'val_family_f1': []
    }
    ...
    for epoch in pbar:
        # Train
        train_loss, train_class_acc, train_order_acc, train_family_acc, train_class_loss, train_order_loss, train_family_loss = train_epoch(
            model, train_loader, criterion, optimizer, Config.DEVICE
        )
        
        # Validate
        val_loss, val_class_acc, val_order_acc, val_family_acc, val_class_f1, val_order_f1, val_family_f1, val_class_loss, val_order_loss, val_family_loss = validate_epoch(
        ...
        is_best = val_family_f1 > best_val_f1
        if is_best:
            best_val_f1 = val_family_f1
            best_epoch = epoch + 1
            patience_counter = 0
            ...
            }, Config.CHECKPOINT_DIR / 'best_H_COF_model.pth')
            
            pbar.set_postfix({'best_f1': f'{val_family_f1:.4f}', 'epoch': epoch+1})
        else:
            patience_counter += 1
    ...
    print(f"\n✅ Training complete: Best Family F1={best_val_f1:.4f} at epoch {best_epoch}/{len(history['train_loss'])}")
    
    # Save training history (plotting is done separately in analyze/)
    history['best_val_family_f1'] = best_val_f1
    history['best_epoch'] = best_epoch
    
    history_path = Config.LOG_DIR / 'H_COF__training_history.json'
