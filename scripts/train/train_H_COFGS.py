#!/usr/bin/env python3
"""
DiatomScanNet H-COFGS Training
Level 5: Class → Order → Family → Genus → Species
"""
...
class Config:
    ...
    # Pretrained model (use H-COFG hierarchical model)
    PRETRAINED_MODEL_PATH = CHECKPOINT_DIR / "best_H_COFG_model.pth"
...
def load_and_prepare_data():
    ...
    Config.NUM_SPECIES = len(species_encoder.classes_)
    ...
    masks = build_hierarchy_masks_from_dataframe(
        df, class_encoder, order_encoder, family_encoder, 
        genus_encoder, species_encoder=species_encoder
    )
    ...
    Config.M_GENUS_SPECIES = masks['M_genus_species']
    ...
    M_genus_species = masks['M_genus_species'].numpy()
    ...
    gs_valid = M_genus_species.sum()
    gs_total = Config.NUM_GENERA * Config.NUM_SPECIES
    
    return df, class_encoder, order_encoder, family_encoder, genus_encoder, species_encoder

def create_data_loaders(df, class_encoder, order_encoder, family_encoder, genus_encoder, species_encoder):
    """Create data loaders"""
    ...
    train_dataset = HCOFGSDataset(train_df, class_encoder, order_encoder, family_encoder, genus_encoder, species_encoder, train_transforms)
    val_dataset = HCOFGSDataset(val_df, class_encoder, order_encoder, family_encoder, genus_encoder, species_encoder, val_transforms)
    test_dataset = HCOFGSDataset(test_df, class_encoder, order_encoder, family_encoder, genus_encoder, species_encoder, val_transforms)
...
def train_epoch(model, train_loader, criterion, optimizer, device):
    ...
    for images, class_labels, order_labels, family_labels, genus_labels, species_labels in train_loader:
        ...
        optimizer.zero_grad()
        class_logits, order_logits, family_logits, genus_logits, species_logits, class_probs, order_probs, family_probs, genus_probs = model(images)
        ...
        # Species loss (Masked Focal Loss)
        m_species = Config.M_GENUS_SPECIES.to(device)[genus_labels]
        species_loss = masked_focal_loss(species_logits, species_labels, m_species, alpha=TrainingConfig.FOCAL_ALPHA, gamma=TrainingConfig.FOCAL_GAMMA)
        
        # Get loss weights from config
        weights = TrainingConfig.LOSS_WEIGHTS["H-COFGS"]
        ...
        species_loss_total += species_loss.item()
        
        with torch.no_grad():
            pred_class, pred_order, pred_family, pred_genus, pred_species = hierarchical_predict(
                class_logits.detach(), order_logits.detach(), 
                family_logits=family_logits.detach(), genus_logits=genus_logits.detach(), 
                species_logits=species_logits.detach(),
                M_class_order=Config.M_CLASS_ORDER, M_order_family=Config.M_ORDER_FAMILY, 
                M_family_genus=Config.M_FAMILY_GENUS, M_genus_species=Config.M_GENUS_SPECIES
            )
        ...
        species_correct += (pred_species == species_labels).sum().item()
    ...
    avg_species_loss = species_loss_total / num_batches
...
def validate_epoch(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for images, class_labels, order_labels, family_labels, genus_labels, species_labels in val_loader:
            images = images.to(device)
            class_labels = class_labels.to(device)
            order_labels = order_labels.to(device)
            family_labels = family_labels.to(device)
            ...
            all_species_preds.extend(pred_species.cpu().numpy())
    ...
    species_f1 = f1_score(all_species_labels, all_species_preds, average='weighted', zero_division=0)
...
def main():
    parser = argparse.ArgumentParser(description='Train H-COFGS hierarchical model')
    ...
    train_loader, val_loader, test_loader = create_data_loaders(
        df, class_encoder, order_encoder, family_encoder, genus_encoder, species_encoder
    )
    ...
    history = {
        ...
        'train_species_loss': [], 'val_species_loss': [],
        'train_class_acc': [], 'val_class_acc': [],
        'train_order_acc': [], 'val_order_acc': [],
        'train_family_acc': [], 'val_family_acc': [],
        'train_genus_acc': [], 'val_genus_acc': [],
        'train_species_acc': [], 'val_species_acc': [],
        'val_class_f1': [], 'val_order_f1': [], 'val_family_f1': [], 'val_genus_f1': [], 'val_species_f1': []
    }
    ...
    for epoch in pbar:
        ...
        history['train_species_acc'].append(train_species_acc)
        history['val_species_acc'].append(val_species_acc)
        ...
        history['val_species_f1'].append(val_species_f1)
        
        # Update learning rate
        scheduler.step(val_loss)  # ReduceLROnPlateau needs val_loss
        
        # Check if best model
        is_best = val_species_f1 > best_val_f1
        if is_best:
            best_val_f1 = val_species_f1
            best_epoch = epoch + 1
            patience_counter = 0
            ...
            torch.save({
                ...
                'val_species_f1': val_species_f1,
                'val_species_acc': val_species_acc,
                ...
                'species_names': list(species_encoder.classes_),
                ...
                'M_genus_species': Config.M_GENUS_SPECIES,
                'checkpoint_schema_version': CHECKPOINT_SCHEMA_VERSION
            }, Config.CHECKPOINT_DIR / 'best_H_COFGS_model.pth')
            
            pbar.set_postfix({'best_f1': f'{val_species_f1:.4f}', 'epoch': epoch+1})
        else:
            patience_counter += 1
        ...
        if patience_counter >= Config.PATIENCE:
            break
    print(f"\n✅ Training complete: Best Species F1={best_val_f1:.4f} at epoch {best_epoch}/{len(history['train_loss'])}")
    
    # Save training history
    history['best_val_species_f1'] = best_val_f1
    history['best_epoch'] = best_epoch
    
    history_path = Config.LOG_DIR / 'H_COFGS__training_history.json'
