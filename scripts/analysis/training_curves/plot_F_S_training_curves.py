#!/usr/bin/env python3
"""
Plot F-S (Flat Species-only) Training Curves
"""
import json
...
def plot_f_s_training_curves(
    ...
    train_accs = history.get('train_species_acc', history.get('train_accs', history.get('train_acc', [])))
    val_accs = history.get('val_species_acc', history.get('val_accs', history.get('val_acc', [])))
    val_f1s = history.get('val_species_f1', history.get('val_f1s', history.get('val_f1', [])))
    ...
    # Calculate spacing - need more gap for F-S due to 3-column layout
    total_height = 8.5
    ...
    fig.suptitle('F-S: Flat Species-Only Model Training Curves', 
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate F-S training curves')
    ...
    history_file = args.history or LOG_DIR / 'F_S__training_history.json'
    output_file = args.output or REPORT_DIR / 'F_S__training_curves.png'
    
    print(f"📊 Generating F-S training curves...")
